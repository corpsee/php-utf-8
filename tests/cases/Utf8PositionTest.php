...
require_once PHP_UTF_8_DIR . '/utils/position.php';
...
class Utf8PositionTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $str = 'testing';
        $this->assertEquals(3, utf8\byte_position($str, 3));
        $this->assertEquals(array(3, 4), utf8\byte_position($str, 3, 4));
        $this->assertEquals(0, utf8\byte_position($str, -1));
        $this->assertEquals(7, utf8\byte_position($str, 8));
    }

    public function test_multibyte_char_to_byte()
    {
        $str = 'Iñtërnâtiônàlizætiøn';
        $this->assertEquals(4, utf8\byte_position($str, 3));
        $this->assertEquals(array(4, 7), utf8\byte_position($str, 3, 5));
        $this->assertEquals(0, utf8\byte_position($str, -1));
        $this->assertEquals(27, utf8\byte_position($str, 28));
    }

    // Tests for utf8\locate_current_chr & utf8\locate_next_chr
    public function test_singlebyte()
    {
        ...
        foreach ($tests as $test) {
            $this->assertEquals($test[2], utf8\locate_current_chr($test[0], $test[1]));
        }
        ...
        foreach ($tests as $test) {
            $this->assertEquals($test[2], utf8\locate_next_chr($test[0], $test[1]));
        }
    }
